...
def send_email(receiver_email, frame, max_retries=3, delay=5):
    ...
    while attempt < max_retries:
        try:
            ...
            server.login('akbarsheikh27012701@gmail.com', 'cwse ndpo xfyu ypvu')  # Update with your credentials(sender_mail,password)

            # Create the email
            msg = MIMEMultipart()
            msg['From'] = 'akbarsheikh27012701@gmail.com'  # Update with your email
            msg['To'] = receiver_email
            msg['Subject'] = 'Fire Detected'

            # Encode the frame as an image
            _, buffer = cv2.imencode('.jpg', frame)
            img_data = buffer.tobytes()  # Convert the buffer to bytes(Image to Bytes)
            ...
            for box, track_id, class_id, mask in zip(boxes, track_ids, class_ids, masks):
                ...
                if mask.size > 0:
                    ...
                    if 'fire' in c:
                        ...
                        receiver_email = "akbarsheikh27012701@gmail.com"  # Update with the actual receiver email
